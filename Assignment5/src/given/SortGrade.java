...
public class SortGrade<K extends Comparable<K>> {
  ...
  public static boolean comparesAndSwaps(AbstractArraySort<Integer> alg, String type)
    ...
    if(alg.toString().equals("Mergesort")){
      type = type +"";
    }
    BufferedReader fileReader = new BufferedReader(new FileReader("compare/" + alg.toString() + "_" + type + ".txt"));
    int swaps = Integer.parseInt(fileReader.readLine());
    int compares = Integer.parseInt(fileReader.readLine());
    fileReader.close();
    if(!(swaps == alg.getSwaps() && compares == alg.getCompares())){
      System.out.println(alg.name + " swaps " + swaps + " " + alg.getSwaps() +"compares "+ compares + " "+ alg.getCompares());
    }
    return swaps == alg.getSwaps() && compares == alg.getCompares();
  }
